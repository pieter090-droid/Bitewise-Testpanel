-- ---------------------------------------------------------------------------
-- Bugfix: de harde poort in calculate_swap_score keek alleen naar
-- category_cluster (7 brede emmers). Gebleken (via "Chocolade mousse"):
-- bijna elk snack_type spreidt zich over 2-4 verschillende category_cluster-
-- waarden (bv. kaas -> hartig/zuivel, ijs -> zoet/zuivel, sap -> drank/
-- fruit_groente). Twee bijna-identieke producten met hetzelfde snack_type
-- kregen zo alsnog nul kans op een swap omdat de AI ze toevallig anders
-- clusterde. snack_type is het fijnere, betrouwbaardere signaal (~22
-- waarden i.p.v. 7) en hoort dus leidend te zijn in de poort; category_cluster
-- is alleen nog een vangnet als snack_type ontbreekt (product nog niet
-- volledig verrijkt). Zelfde tweetraps-prioriteit als al gebruikt bij het
-- ophalen van kandidaten (zie snackswap_service.dart getCandidatesForCluster).
-- ---------------------------------------------------------------------------
...
begin
  ...
  select p.kcal_100g, p.sugar_100g, p.protein_100g, p.fiber_100g, p.salt_100g, p.allergens,
         pf.category_cluster, pf.snack_type, pf.processing_quality_score, pf.data_quality_score, pf.ai_confidence
    into f
  ...
  -- Harde poort: snack_type leidend (fijn), category_cluster alleen als
  -- vangnet wanneer snack_type nog ontbreekt. Nooit chocolade -> komkommer.
  v_gate_passes := case
