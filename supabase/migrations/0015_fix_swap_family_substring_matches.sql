-- ---------------------------------------------------------------------------
-- Bugfix ronde 2 op compute_swap_family_fields(). Steekproef over alle
-- emmers vond 4 concrete misclassificaties:
--  1. "cola" matchte als substring binnen "Chocolade" (geen woordgrens)
--     -> chocoladeproducten belandden in soft_drinks.
--  2. "cheese" matchte als substring binnen "Cheesecake" (geen woordgrens)
--     -> een dessert belandde in cheese_snacks.
--  3. OFF's gecombineerde pnns_groups_2 "Biscuits and cakes" dekt ook
--     desserts zoals tiramisu -> AH Tiramisu belandde in cookies_biscuits
--     i.p.v. dairy_desserts (dessert-checks liepen te laat in de prioriteit).
--  4. OFF's gecombineerde pnns_groups_1 "Fat and sauces" dekt zowel vetten
--     (boter/margarine) als sauzen -> roomboter belandde in sauces_dips.
-- Fix: woordgrenzen (\m...\M) op korte/substring-gevoelige trefwoorden,
-- pnns_groups_1 en pnns_groups_2 apart behandelen i.p.v. samengevoegd,
-- dessert-checks (ijs/dairy) vóór cookie-checks, expliciete boter-uitsluiting.
-- ---------------------------------------------------------------------------
...
begin
  ...
  elsif c ~* 'tiramisu|dairy-desserts'
  ...
  elsif n ~* 'yoghurt|skyr|kwark|quark'
        and not (n ~* '\mdrink\M|drank|drinkyoghurt' or c ~* 'drinkable') then
  ...
  elsif n ~* '\mcola\M|frisdrank|\msoda\M|limonade|energy ?drink' then
...
-- Herbereken alle rijen opnieuw met de gecorrigeerde functie.
