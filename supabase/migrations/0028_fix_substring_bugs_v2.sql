-- ---------------------------------------------------------------------------
-- Bugfix ronde op 0027: dezelfde klasse fouten als eerder deze sessie.
--  1. "gebak" matchte binnen "Ovengebakken" -> kipfilet werd cakes_pastries.
--  2. "jam" matchte binnen "Jambalaya" en "Ajam" (Indonesisch voor kip) ->
--     rijstgerechten werden jams_fruit_spreads.
--  3. "honing"/"honey" stond te vroeg in de keten en ving ook honing-
--     gearomatiseerde producten die feitelijk iets anders zijn (kaas met
--     honing, ham met honing, honing-mosterdsaus) -> nu pas gecheckt NADAT
--     kaas/vlees/sauzen al hun kans hebben gehad.
-- Fix: woordgrenzen (\m...\M) en honey_syrups later in de prioriteitsketen.
-- ---------------------------------------------------------------------------
...
begin
  ...
  elsif n ~* '\mjam\M|confiture|marmelade' then
  ...
  -- Honing/siroop pas HIER checken: eerst kaas/vlees/saus hun kans geven,
  -- want "honing" is vaak alleen een smaakwoord (kaas met honing, ham met
  -- honing, honing-mosterdsaus) i.p.v. het product zelf.
  elsif n ~* '\mhoning\M|\mhoney\M|\msiroop\M|\msyrup\M|\magave\M|maple syrup|ahornsiroop' then
    return 'honey_syrups';
