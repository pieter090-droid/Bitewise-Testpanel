-- ---------------------------------------------------------------------------
-- Bugfix ronde 3: OFF tagt gewone yoghurt óók met categories_tags "en:dairy-
-- desserts" (yoghurt is in OFF's hiërarchie een sub-type van dairy dessert).
-- Omdat dairy_desserts vóór yoghurt_skyr_quark werd gecheckt, belandde alle
-- gewone yoghurt (Griekse yoghurt, magere yoghurt, enz.) in dairy_desserts
-- i.p.v. de specifiekere yoghurt-emmer. Fix: yoghurt-check eerst (specifieker
-- signaal), dairy_desserts alleen nog als vangnet erna. Tevens: "Yoghurt
-- Bieslook Dressing" (een hartige yoghurtdressing) werd ten onrechte als
-- lepelbare yoghurt-dessert geclassificeerd -- uitgesloten met een guard.
-- ---------------------------------------------------------------------------
...
begin
  if n ~* 'pindakaas|peanut butter|amandelpasta|notenpasta|cashewpasta' then
    ...
    return query select 'ice_cream_desserts', 'dessert_cup', 'spoonable', array['dessert','treat'];
        ...
        and not (n ~* '\mdrink\M|drank|drinkyoghurt|dressing|saus|sauce|\mdip\M' or c ~* 'drinkable') then
