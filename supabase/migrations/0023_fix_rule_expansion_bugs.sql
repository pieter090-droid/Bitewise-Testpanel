-- ---------------------------------------------------------------------------
-- Bugfixes op 0022:
--  1. `\b` (Perl/PCRE-woordgrens) bestaat niet in Postgres se regex-dialect --
--     stilzwijgend genegeerd (geen fout, gewoon nooit match). Daardoor gaf
--     fresh_fruit/hot_beverages/alcohol_drinks/bread_bakery/cold_cuts/
--     soft_drinks 0 of te weinig resultaten op de patronen die \b gebruikten.
--     Fix: overal \m/\M (Postgres' eigen woordgrens-syntax), zoals al
--     consistent gebruikt in 0013-0021.
--  2. KitKat (een chocoladereep) belandde in candy_sweets via de te brede
--     `categories_tags ~* 'candies|sugar.confectionery'`-check -- OFF tagt
--     chocoladerepen ook met "en:confectioneries". Fix: candy_sweets steunt
--     nu alleen nog op specifieke naam-trefwoorden, geen brede tag-match.
--  3. fresh_vegetables ving via de brede `pnns_groups_2 ~* 'vegetables'` ook
--     bewerkte producten (uienringen, tomatenpulp, wokmix) -- diezelfde OFF-
--     brede-pnns-valkuil als eerder bij "Biscuits and cakes"/"Fat and
--     sauces". Fix: specifieke groentenamen i.p.v. de brede pnns-match.
-- ---------------------------------------------------------------------------
