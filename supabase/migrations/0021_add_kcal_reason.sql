-- ---------------------------------------------------------------------------
-- calculate_swap_score gaf een reden-string voor "minder suiker" en "meer
-- eiwit", maar niet voor "minder kcal" -- terwijl dat sinds de groep-
-- herindeling een van de 4 hoofdgroepen is. Een kandidaat die alleen op kcal
-- verbetert kwam zo zonder toelichting in de UI terecht. Zelfde fix als in
-- SwapScoreCalculator (Dart).
-- ---------------------------------------------------------------------------
...
begin
  ...
  if f.kcal_100g is not null and t.kcal_100g is not null and f.kcal_100g > 0 then
    ...
    if t.kcal_100g < f.kcal_100g then
      v_reasons := v_reasons || format('%s%% minder kcal', round((1 - t.kcal_100g/f.kcal_100g)*100));
    end if;
  end if;
